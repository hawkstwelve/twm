...
tippecanoe -f -o "$TMP_DIR/boundary_country.mbtiles" -l boundaries -Z0 -z10 --buffer=8 --detect-shared-borders --no-feature-limit --no-tile-size-limit "$BUILD_DIR/country_lines.geojson"
...
tippecanoe -f -o "$TMP_DIR/hydro_coastline.mbtiles" -l hydro -Z0 -z10 --buffer=8 --no-feature-limit --no-tile-size-limit "$BUILD_DIR/coastline_lines.geojson"
